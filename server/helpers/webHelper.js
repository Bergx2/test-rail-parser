...
const getAllSuitesAndCases = async data => {
  ...
  const testrailProjectWebSuites = await getResources({
    id: getProjectId(webProject),
    endpoint: 'get_suites'
  });
  let testrailProjectNativeSuites = [];

  if (appProject) {
    testrailProjectNativeSuites = await getResources({
      id: getProjectId(appProject),
      endpoint: 'get_suites'
    });
  }
...
const parseTest = async data => {
  ...
  const { createProjectRepoCases } = getPreparedProjectRepoCases({
